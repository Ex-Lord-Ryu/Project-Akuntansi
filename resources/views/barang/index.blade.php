        ...
        <div class="mt-6 bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg p-6">
            <div class="flex justify-end mb-4">
                <form action="{{ route('barang.index') }}" method="GET" class="flex items-center">
                    <input type="text" name="search" placeholder="Cari nama barang..." value="{{ request()->query('search') }}" class="form-input rounded-l border-0">
            ...
            <div class="flex justify-between mb-4">
                ...
                <div class="flex justify-end space-x-2">
                    <a href="{{ route('barang.create') }}" class="btn btn-light border-custom">
                    ...
                    <tbody class="bg-white dark:bg-gray-800 divide-y divide-gray-200" id="table-body">
                        @foreach ($barang as $item)
                            <tr data-id="{{ $item->id }}" class="hover:bg-gray-100 dark:hover:bg-gray-700">
                                ...
                                <td class="px-6 py-4 whitespace-no-wrap text-sm font-medium text-gray-900 dark:text-gray-100">
                                    <img src="{{ asset('storage/images/' . $item->image) }}" alt="{{ $item->nama }}" width="100" onerror="this.onerror=null;this.src='{{ asset('assets/images/default.jpg') }}';">
                                </td>
                            </tr>
                        @endforeach
                    </tbody>
...
<script>
    document.addEventListener('DOMContentLoaded', function () {
        ...
        document.getElementById('edit-button').addEventListener('click', function () {
            if (selectedRowId) {
                window.location.href = `/barang/${selectedRowId}/edit`;
            }
        });
        ...
        document.getElementById('delete-button').addEventListener('click', function () {
            if (selectedRowId) {
                if (confirm('Data yang dihapus tidak dapat direstorasi?')) {
                    ...
                    form.method = 'POST';
                    form.action = `/barang/${selectedRowId}`;
                    form.innerHTML = `
                        @csrf
                        @method('DELETE')
                    `;
