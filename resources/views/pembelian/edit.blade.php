        ...
        <div class="mt-6 bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg p-6">
            <form action="{{ route('pembelian.update', $pembelian->id) }}" method="POST">
                ...
                <div class="mb-4">
                    ...
                    <select name="id_vendor" id="id_vendor" class="form-control mt-1 block w-full @error('id_vendor') is-invalid @enderror" required>
                        @foreach ($vendors as $vendor)
                            <option value="{{ $vendor->id }}" {{ old('id_vendor', $pembelian->id_vendor) == $vendor->id ? 'selected' : '' }}>
                                {{ $vendor->nama }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div class="mb-4">
                    ...
                    <input type="date" name="tgl_pembelian" id="tgl_pembelian" class="form-control mt-1 block w-full @error('tgl_pembelian') is-invalid @enderror"
                ...
                <div class="mb-4">
                    ...
                    <select name="id_status" id="id_status" class="form-control mt-1 block w-full @error('id_status') is-invalid @enderror" required>
                        @foreach ($statuses as $status)
                            <option value="{{ $status->id }}" {{ old('id_status', $pembelian->id_status) == $status->id ? 'selected' : '' }}>
                                {{ $status->nama_status }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div class="mb-4">
                    ...
                    <select name="id_pengirim" id="id_pengirim" class="form-control mt-1 block w-full @error('id_pengirim') is-invalid @enderror">
                        ...
                        @foreach ($pengirims as $pengirim)
                            <option value="{{ $pengirim->id }}" {{ old('id_pengirim', $pembelian->id_pengirim) == $pengirim->id ? 'selected' : '' }}>
                                {{ $pengirim->jenis }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div class="mb-4">
                    ...
                    <input type="date" name="tgl_pengiriman" id="tgl_pengiriman" class="form-control mt-1 block w-full @error('tgl_pengiriman') is-invalid @enderror"
                    ...
                    @foreach ($pembelian->items as $index => $item)
                        <div class="item mb-4">
                            ...
                            <div class="mb-4">
                                ...
                                <select name="items[{{ $index }}][id_barang]" id="items[{{ $index }}][id_barang]" class="form-control mt-1 block w-full @error('items[{{ $index }}][id_barang]') is-invalid @enderror" required>
                                    @foreach ($barangs as $barang)
                                        <option value="{{ $barang->id }}" {{ old("items.$index.id_barang", $item->id_barang) == $barang->id ? 'selected' : '' }}>
                                            {{ $barang->nama }}
                                        </option>
                                    @endforeach
                                </select>
                            ...
                            <div class="mb-4">
                                ...
                                <select name="items[{{ $index }}][id_warna]" id="items[{{ $index }}][id_warna]" class="form-control mt-1 block w-full @error('items[{{ $index }}][id_warna]') is-invalid @enderror">
                                    <option value="">Pilih Warna</option>
                                    @foreach ($warnas as $warna)
                                        <option value="{{ $warna->id }}" {{ old("items.$index.id_warna", $item->id_warna) == $warna->id ? 'selected' : '' }}>
                                            {{ $warna->warna }}
                                        </option>
                                    @endforeach
                                </select>
                            ...
                            <div class="mb-4">
                                ...
                                <input type="number" name="items[{{ $index }}][harga]" id="items[{{ $index }}][harga]" class="form-control mt-1 block w-full @error('items[{{ $index }}][harga]') is-invalid @enderror" min="0" step="1" onkeydown="return event.keyCode !== 38 && event.keyCode !== 40" onwheel="return false;" required value="{{ old("items.$index.harga", $item->harga) }}">
